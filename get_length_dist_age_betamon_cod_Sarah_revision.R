...
get.length.dist.age<-function(age, N.vec, F.vec, M.vec, l.breaks, l.mids, G, linf, k, t0, sd.l.age, n.seasons, a, b){
  ...
  prop.l<-cdf.l1-cdf.l
  ## reviewer Andre Punt recommended we restrict the upper bin; we chose 95%-ile, which is 24 cm
  prop.l.new<-prop.l
  prop.l.new[23:198]<-0
  prop.new<-prop.l.new/sum(prop.l.new)
  ## number in first season, age 1 
  #n.l<-prop.l*N1 we replace this
  n.l<-prop.new*N1
  ## age 1
  if(age==1){
    return(n.l)
