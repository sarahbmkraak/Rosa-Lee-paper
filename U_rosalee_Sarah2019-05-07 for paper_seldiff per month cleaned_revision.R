...
## new since 07.05.2019 we have length starting with 1 cm instead of 2 cm
l.breaks<-seq(1,200, by=1)
...
## Total Biomass at the start of the first season each year
B.statusquo<-get.B(N.array=N.statusquo,n.years,n.ages,n.seasons, l.mids=l.mids)
B.statusquo.sum <- apply(B.statusquo[,,1,], 3, sum)

## SSB at the start of the first season each year
SSB.statusquo<-get.SSB(B.array=B.statusquo,n.years,n.ages,n.seasons, l.mids=l.mids)
SSB.statusquo.sum<-apply(SSB.statusquo[,,1,],3,sum)
...
B.gearchange.sum <- apply(B.gearchange[,,1,],3, sum)

## SSB
SSB.gearchange<-get.SSB(B.array=B.gearchange,n.years,n.ages,n.seasons, l.mids=l.mids)
SSB.gearchange.sum<-apply(SSB.gearchange[,,1,],3,sum)
...
B.Fchange.sum <- apply(B.Fchange[,,1,], 3, sum)

## SSB
SSB.Fchange<-get.SSB(B.array=B.Fchange,n.years,n.ages,n.seasons, l.mids=l.mids)
SSB.Fchange.sum<-apply(SSB.Fchange[,,1,],3,sum)
...
for (i in 1:10){
  SSB[i] <- sum(N.at.age.jki[,1,(n.ages+1)+i]*mean_weight_jk[,1]*mat_prop[,1]) 
...
for (i in pred.years){
  ...
  for (k in 1:n.seasons){
    ...
    for(j in 2:n.ages){
      ...
      if (k == 1){
        #new since 07.07.2019, because in year 22 season 1 it should still be status quo
        if (i == 1)
        {
          N.at.age.jki.gear[j,1,i+n.ages+1] <- N.at.age.jki[j,1,i+n.ages+1]  #season 1 in year 22 still status quo
        }
        else
        {
...
for (i in 1:10){
  SSB.gear[i] <- sum(N.at.age.jki.gear[,1,n.ages+1+i]*mean_weight_jk[,1]*mat_prop[,1])
  ...
  for (k in 1:n.seasons){
    ...
    for(j in 2:n.ages){
      ...
      if (k == 1){
        ...
        if (i == 1) 
        {
          N.at.age.jki.newF[j,1,i+n.ages+1] <- N.at.age.jki[j,1,i+n.ages+1]  #season 1 in year 22 still status quo
        }
        else{
...
for (i in 1:10){
  SSB.newF[i] <- sum(N.at.age.jki.newF[,1,n.ages+1+i]*mean_weight_jk[,1]*mat_prop[,1])
...
## on 07.05.2019 this was changed. Our calculations are for the start of a month. So we want for the start of month 1
...
plot(SSB.statusquo.sum[20:31]/1000,type="n", col="darkblue", xlim=c(start.year,pred.year.date[length(pred.year.date)]), ylim=c(0,25), xaxt="n", yaxt="n", ann=F)
axis(side=2, at=seq(0,200,by=5), font=1,cex.axis=0.8, las=1)
abline(v=start.year:pred.year.date[length(pred.year.date)], col="grey")
abline(h=seq(0,100,by=5), col="grey")
# static
lines(pred.year.date, SSB[20:31]/1000, lwd=1)
lines(pred.year.date, SSB.gear[20:31]/1000, lwd=1.5, lty=5)
# dynamic
lines(pred.year.date, SSB.gearchange.sum[20:31]/1000, lwd=1.5, lty=3)
lines(pred.year.date,SSB.statusquo.sum[20:31]/1000, lwd=1.5, lty=1)
mtext(side=2,line=2.2, "SSB ('000 t)")
mtext('(a)', side = 3, line = -1.2, adj = 0.05, cex = 1, col = "black")

plot(pred.year.date,Lw.statusquo.sum[20:31]/1000,type="n", col="darkblue", xlim=c(start.year,pred.year.date[length(pred.year.date)]), ylim=c(0,15), xaxt="n", yaxt="n", ylab="landings",ann=F)
axis(side=4, at=seq(0,200,by=5), font=1,cex.axis=0.8, las=1)
abline(v=start.year:pred.year.date[length(pred.year.date)], col="grey")
abline(h=seq(0,100,by=5), col="grey")
lines(pred.year.date, landings[20:31]/1000, lwd=1)
lines(pred.year.date, landings.gear[20:31]/1000, lwd=1.5, lty=5)
lines(pred.year.date, Lw.gearchange.sum[20:31]/1000, lwd=1.5, lty=3)
lines(pred.year.date,Lw.statusquo.sum[20:31]/1000, lwd=1.5, lty=1)
mtext(side=4,line=2.2, "Catch above MCRS ('000 t)")
mtext('(b)', side = 3, line = -1.2, adj = 0.05, cex = 1, col = "black")

plot(pred.year.date,Dw.statusquo.sum[20:31]/100,type="n", col="darkblue", xlim=c(start.year,pred.year.date[length(pred.year.date)]), ylim=c(0,15), xaxt="n", yaxt="n", ylab="discards",ann=F)
axis(side=2, at=seq(0,2000,by=5), font=1,cex.axis=0.8, las=1)
axis(side=1, at=seq(0,40,by=1), font=1,cex.axis=0.8)
abline(v=start.year:pred.year.date[length(pred.year.date)], col="grey")
abline(h=seq(0,1000,by=5), col="grey")
lines(pred.year.date, discards[20:31]/100, lwd=1)
lines(pred.year.date, discards.gear[20:31]/100, lwd=1.5, lty=5)
lines(pred.year.date, Dw.gearchange.sum[20:31]/100, lwd=1.5, lty=3)
lines(pred.year.date,Dw.statusquo.sum[20:31]/100, lwd=1.5, lty=1)
mtext(side=2,line=2.2, "Catch below MCRS ('00 t)")
mtext('(c)', side = 3, line = -1.2, adj = 0.05, cex = 1, col = "black")
...
axis(side=4, at=seq(0,200,by=20), font=1,cex.axis=0.8, las=1)
axis(side=1, at=seq(0,40,by=1), font=1,cex.axis=0.8)
abline(h=seq(0,200,by=10), col="grey")
abline(v=start.year:pred.year.date[length(pred.year.date)], col="grey")
for (j in 1:15){
...
mtext('(d)', side = 3, line = -1.2, adj = 0.05, cex = 1, col = "black")
...
# static
lines(pred.year.date, SSB[20:31]/1000, lwd=1)
lines(pred.year.date, SSB.newF[20:31]/1000, lwd=1.5, lty=5)
# dynamic
lines(pred.year.date, SSB.Fchange.sum[20:31]/1000, lwd=1.5, lty=3)
lines(pred.year.date,SSB.statusquo.sum[20:31]/1000, lwd=1.5, lty=1)
mtext(side=2,line=2.2, "SSB ('000 t)")
mtext('(a)', side = 3, line = -1.2, adj = 0.05, cex = 1, col = "black")
...
abline(h=seq(0,100,by=5), col="grey")
lines(pred.year.date, landings[20:31]/1000, lwd=1)
lines(pred.year.date, landings.newF[20:31]/1000, lwd=1.5, lty=5)
lines(pred.year.date, Lw.Fchange.sum[20:31]/1000, lwd=1.5, lty=3)
lines(pred.year.date,Lw.statusquo.sum[20:31]/1000, lwd=1.5, lty=1)
mtext(side=4,line=2.2, "Catch above MCRS ('000 t)")
mtext('(b)', side = 3, line = -1.2, adj = 0.05, cex = 1, col = "black")
...
abline(h=seq(0,1000,by=5), col="grey")
lines(pred.year.date, discards[20:31]/100, lwd=1)
lines(pred.year.date, discards.newF[20:31]/100, lwd=1.5, lty=5)
lines(pred.year.date, Dw.Fchange.sum[20:31]/100, lwd=1.5, lty=3)
lines(pred.year.date,Dw.statusquo.sum[20:31]/100, lwd=1.5, lty=1)
mtext(side=2,line=2.2, "Catch below MCRS ('00 t)")
mtext('(c)', side = 3, line = -1.2, adj = 0.05, cex = 1, col = "black")
...
axis(side=2, at=seq(0,200,by=20), font=1,cex.axis=0.8, las=1)
axis(side=1, at=seq(0,40,by=1), font=1,cex.axis=0.8)
...
axis(side=2, at=seq(0,350,by=20), font=1,cex.axis=0.8, las=1)
axis(side=1, at=seq(0,40,by=1), font=1,cex.axis=0.8)
